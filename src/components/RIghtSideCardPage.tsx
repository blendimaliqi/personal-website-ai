...
To achieve this, I am utilizing a mix of modern and legacy technologies, with the specific tech stack depending on each application's current state. For completely new frontends, we are developing modern applications using React. These represent our efforts to build fresh, state-of-the-art user interfaces from the ground up.
For existing applications, we are working with and improving legacy technologies such as Java Wicket, Velocity templates, and JSP templates. This involves maintaining and enhancing these systems to extend their functionality and improve user experience without a complete rewrite. All the applications are built on Spring Boot backends.

This flexible approach ensures seamless integration with existing systems while allowing for modernization where possible and appropriate. It also requires a deep understanding of both current best practices and legacy systems, as well as adherence to Mastercard's UI/UX standards.
